...
load_dotenv()
app=FastAPI(title="Master XCloud API",version="2.1")
...
async def reset(data:OpIn, authorization:str|None=Header(default=None)):
    ...
    async def op(page):
        ...
        # RESET = EXCLUIR -> ATIVAR -> DNS
        #
        # A mesma função delete_device() usada no botão "Excluir MAC"
        # é reutilizada aqui, sem alterar seu comportamento.
        #
        # Quando o aparelho está Active, a primeira chamada pode apenas
        # desativá-lo e retornar erro ao não encontrar Delete imediatamente.
        # Nesse caso esperamos a atualização do painel e chamamos EXCLUIR
        # novamente. Já Deactive, a função de exclusão que funciona
        # isoladamente encontra Delete e conclui a remoção.
        try:
            await delete_device(page,dev)
        except XCloudError as e:
            msg=str(e).lower()

            retry_after_deactivate = any(x in msg for x in [
                "delete",
                "exclus",
                "desativ",
                "deactiv",
            ...
            if not retry_after_deactivate:
                raise

            await asyncio.sleep(2.0)

            # Segunda chamada usa EXATAMENTE a mesma função Excluir.
            await delete_device(page,dev)

        # Só chega aqui depois que a exclusão terminou.
        await asyncio.sleep(1.0)

        # Agora segue o mesmo fluxo da função Ativar MAC.
        await add_device(page,dev)
        await add_playlist(page,dev,data.playlist.strip())

        return {
            "ok":True,
            "message":"Reset concluído: excluído, recriado e DNS adicionado."
        }
